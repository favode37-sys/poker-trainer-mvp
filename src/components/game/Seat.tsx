...
export function Seat({ position, player, positionLabel, betAmount = 0, isDealer = false, isHero = false, isFolded = false, lastAction }: SeatProps) {
    ...
    return (
        <div className="relative flex flex-col items-center justify-center">
            {/* PLAYER TOKEN (gets grayed out when folded) */}
            <div className={cn("transition-all duration-500", isFolded ? "opacity-50 grayscale" : "opacity-100")}>
                <div className="relative group">
                    ...
                    <div className={cn(
                        ...
                        "bg-slate-100 shadow-[inset_2px_2px_5px_rgba(255,255,255,1),inset_-2px_-2px_5px_rgba(0,0,0,0.1),0_5px_10px_rgba(0,0,0,0.15)]",
                    ...
                    {/* ACTION BUBBLE (Pop-up) - Show Fold even when folded */}
                    <AnimatePresence>
                        {lastAction && (
                            <motion.div
            ...
            {/* BET CHIPS - Outside grayscale wrapper, stays colorful! */}
            <AnimatePresence>
                {betAmount > 0 && (
